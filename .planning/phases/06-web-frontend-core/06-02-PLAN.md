...
must_haves:
  ...
  key_links:
    ...
    - from: "frontend/src/routes/login.tsx"
      to: "frontend/src/features/auth/components/LoginForm.tsx"
      via: "Page renders form component"
      pattern: "LoginForm"
---
...
Output: Working auth pages that connect to backend APIs, manage auth state in Zustand, and handle token storage via httpOnly cookies.
</objective>
...
<task type="auto">
  <name>Task 3: Create auth pages and routing</name>
  <files>
    frontend/src/routes/login.tsx
    frontend/src/routes/signup.tsx
  ...
  <action>
Create full-page auth routes and set up React Router:
...
4. Update src/App.tsx with routing:
     ...
     return (
       <BrowserRouter>
         <Routes>
           ...
           {/* Placeholder routes - will be added in later plans */}
           <Route path="/catalog" element={<div className="p-4">Catalog (coming soon)</div>} />
  ...
  <verify>
    ...
    grep -r "BrowserRouter" src/App.tsx && echo "Router OK"
    grep -r "LoginPage" src/routes/login.tsx && echo "LoginPage OK"
  </verify>
  <done>Auth pages created, React Router configured</done>
</task>
