...
<task type="auto">
  <name>Task 3: Create auth pages and update router.tsx</name>
  <files>
    frontend/src/routes/login.tsx
    frontend/src/routes/signup.tsx
    frontend/src/routes/reset-password.tsx
  ...
  <action>
Create full-page auth routes and UPDATE the existing router.tsx file from 06-01:
...
4. MODIFY src/routes/router.tsx - Read the existing file and update it to add auth routes:

   IMPORTANT: This task MODIFIES the existing router.tsx file created in Plan 06-01.
   - Preserve the createBrowserRouter structure
   - Add imports for LoginPage, SignupPage, ResetPasswordPage at the top
   - Replace placeholder routes with actual auth page components
   - Keep the catalog placeholder for now (Plan 06-03 will replace it)
    ...
    * All subsequent plans (06-03, 06-04, 06-05) will continue to MODIFY this file
    * by importing additional route components and updating the routes array.
    *
    * Current status:
    * - Auth routes (06-02): Complete
    * - App routes with RootLayout (06-03): Coming next
    *
    * Route structure:
...
IMPORTANT: Do NOT modify App.tsx - it already uses RouterProvider from 06-01. Only update router.tsx by modifying the existing file.
  </action>
  <verify>
    cd frontend && npm run build
