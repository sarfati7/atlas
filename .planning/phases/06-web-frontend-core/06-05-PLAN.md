...
Output: Working dashboard page that requires authentication, shows user stats, and displays a summary of available catalog items. Route added to existing router.tsx with protectedLoader for auth guard.
</objective>
...
<task type="auto">
  <name>Task 3: Create protected loader, dashboard page, and add route to router.tsx</name>
  <files>
    frontend/src/lib/loaders.ts
    frontend/src/routes/dashboard.tsx
    frontend/src/routes/router.tsx
  ...
  <action>
Create route protection and dashboard page, then ADD the route to existing router.tsx:
...
3. MODIFY src/routes/router.tsx - Add dashboard route and loaders to existing router configuration:

   Read the existing router.tsx file first, then ADD these modifications:
   ...
   **Routes to add/modify in the createBrowserRouter array:**

   a) Add `loader: guestLoader` to existing auth routes (/login and /signup)

   b) Add dashboard route to the RootLayout children array (if RootLayout exists from 06-03), OR as a standalone route:
   ```typescript
   {
     path: 'dashboard',
   ...
   c) Add placeholder settings route with protectedLoader:
   ```typescript
   {
     path: 'settings',
   ...
   IMPORTANT: Do NOT replace the entire file. ADD imports and routes to the existing structure created by 06-01 and modified by 06-03. The file should preserve the createBrowserRouter structure and any RootLayout configuration.
  </action>
  <verify>
    cd frontend && npm run build
    grep -r "protectedLoader" src/lib/loaders.ts && echo "Loader OK"
    grep -r "DashboardPage" src/routes/dashboard.tsx && echo "Dashboard OK"
    grep -r "protectedLoader" src/routes/router.tsx && echo "Router OK"
  </verify>
  <done>Protected routes and dashboard page created, route added to router.tsx</done>
</task>
...
- router.tsx uses createBrowserRouter with protectedLoader on dashboard route
</verification>
...
8. Route added to router.tsx with loader, App.tsx unchanged
</success_criteria>
