...
must_haves:
  ...
  key_links:
    ...
    - from: "frontend/src/routes/router.tsx"
      ...
      via: "RootLayout wraps app routes"
      pattern: "RootLayout"
---
...
Output: Working catalog page showing items as cards, tab bar for filtering by type (All/Skills/MCPs/Tools), search input, and sidebar navigation. Routes added to existing router.tsx with RootLayout wrapper.
</objective>
...
<task type="auto">
  <name>Task 3: Create layout components and add catalog routes to router.tsx</name>
  <files>
    frontend/src/components/layout/Sidebar.tsx
    frontend/src/components/layout/RootLayout.tsx
    frontend/src/routes/catalog.tsx
  ...
  <action>
Create layout with sidebar navigation and ADD catalog routes to existing router.tsx:
...
5. Update src/routes/router.tsx - ADD app routes with RootLayout (extend existing router):
   ```typescript
   ...
   export const router = createBrowserRouter([
     ...
     // App routes - with RootLayout (sidebar)
     {
       path: '/',
       element: <RootLayout />,
       children: [
         ...
         {
           // Placeholder - detail page added by 06-04
           path: 'catalog/:id',
           element: <div className="p-6">Detail page (coming in 06-04)</div>,
         },
         {
           // Placeholder - dashboard added by 06-05
           path: 'dashboard',
           element: <div className="p-6">Dashboard (coming in 06-05)</div>,
         },
         {
           path: 'settings',
           element: <div className="p-6">Settings (future)</div>,
         },
       ],
     },
   ])
...
IMPORTANT: Do NOT modify App.tsx - it already uses RouterProvider from 06-01. Only update router.tsx.
  </action>
  <verify>
    cd frontend && npm run build
    grep -r "RootLayout" src/routes/router.tsx && echo "Layout OK"
    grep -r "useCatalogItems" src/routes/catalog.tsx && echo "Catalog OK"
  </verify>
  <done>Layout with sidebar and catalog routes added to router.tsx</done>
</task>
...
- router.tsx uses createBrowserRouter with RootLayout wrapper
</verification>
...
9. Routes added to router.tsx with RootLayout, App.tsx unchanged
</success_criteria>
