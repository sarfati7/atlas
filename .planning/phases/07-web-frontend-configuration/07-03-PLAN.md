...
must_haves:
  ...
  artifacts:
    ...
    - path: "frontend/src/routes/settings.tsx"
      provides: "Settings page with editor"
...
<objective>
Create Settings page with Editor tab, save functionality, and dirty state tracking.

Purpose: Users need a place to edit their claude.md configuration. The Settings page hosts the Monaco editor with save button and unsaved changes warning. This is the core editing experience.

Output:
- Zustand store for draft state (content, isDirty)
- Settings page with Editor tab
- Save button with loading/dirty state
...
<task type="auto">
  <name>Task 2: Create Settings page with Editor tab</name>
  <files>
    frontend/src/routes/settings.tsx
  </files>
  <action>
...
```
  ...
  <verify>
`cd frontend && npx tsc --noEmit` passes
Page has header with save button
Editor tab shows Monaco editor
  </verify>
  <done>
Settings page with:
- Header showing title and Save button
...
5. Try to close tab with unsaved changes - browser warns
</verification>
...
5. Dirty indicator visible when unsaved changes exist
6. beforeunload warns when leaving with unsaved changes
7. Router updated to render SettingsPage
</success_criteria>
